...
class TCNND(nn.Module):
    def __init__(self):
        super(TCNND, self).__init__()
        self.linear = nn.Linear(100, 4*4*256)
        self.decoder = AdversaryModelGen({"channels": 256, "downsampling": 0, "offset": 4})

    def forward(self, z):
        patch = self.linear(z).view(-1, 256, 4, 4)
        return self.decoder(patch)
